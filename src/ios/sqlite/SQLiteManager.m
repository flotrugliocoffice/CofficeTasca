...
- (NSError *)doQuery:(NSString *)sql {
    ...
    @synchronized ([SQLiteManager databaseLock]) {
...
-(NSError*)doTransaction:(NSArray*)queries {
    ...
    @synchronized ([SQLiteManager databaseLock]) {
        ...
        if (openError == nil) {
            ...
            for (NSString* _query in queries) {
                ...
                if(sqlite3_prepare_v2(db, query, -1, &statement, NULL)) {
                    if (sqlite3_step(statement) == SQLITE_ERROR) {
                        ...
                        errorQuery = [self createDBErrorWithDescription:[NSString stringWithCString:errorMsg encoding:NSUTF8StringEncoding]
                                                                andCode:kDBErrorQuery];
                        NSLog(@"execute query: %@",_query);
                        NSLog(@"%@", errorQuery);
                        break; //Esci dal loop se si verificano errori.
                    }
            ...
            if (sqlite3_finalize(statement) != SQLITE_OK) {NSLog(@"SQL Error: %s",sqlite3_errmsg(db));} else {
                NSLog(@"FINALIED STATEMENT");
            }
            //errorQuery = [self closeDatabase];
        }
        else {
...
#pragma mark -
#pragma mark custom sqlite function
